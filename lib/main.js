...
Exec.prototype = {
	...
	config: function(){
		...
		if (this.program.host){
			creds.host = this.program.host;
		}
		// save back to the file if we passed new values
		if (this.program.key || this.program.secret || this.program.host){
			fs.writeFileSync(config_file, JSON.stringify( creds ));
		}
	...
	read : function( domain, options ){
		var sdb = new simpledb.SimpleDB({keyid: this.creds.key ,secret: this.creds.secret, host: this.creds.host });
